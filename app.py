...
with tab1:
    ...
    if st.button("⚡ Predict Churn Risk", key="predict_main"):
        ...
        try:
            ...
            TEAL_RL = colors.HexColor("#0F828C")
            BLUE_RL = colors.HexColor("#065084")
            DARK_RL = colors.HexColor("#1e1e1e")

            title_style = ParagraphStyle("rptitle", parent=styles["Title"],
                                         textColor=TEAL_RL, fontSize=18, spaceAfter=4)
            h2_style    = ParagraphStyle("rph2", parent=styles["Heading2"],
                                         textColor=BLUE_RL, fontSize=13, spaceBefore=10, spaceAfter=4)
            body_style  = ParagraphStyle("rpbody", parent=styles["Normal"],
                                         textColor=DARK_RL, fontSize=11, leading=16)

            tier_txt   = "High" if pct > 66 else ("Medium" if pct > 33 else "Low")
            tier_color = "#dc2626" if pct > 66 else ("#d97706" if pct > 33 else "#16a34a")

            story = [
                Paragraph("Churn Intelligence - Prediction Report", title_style),
                HRFlowable(width="100%", thickness=1, color=TEAL_RL, spaceAfter=8),
                Paragraph("Prediction Summary", h2_style),
                Paragraph(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", body_style),
                Paragraph(f"Churn Probability: <b>{pct:.1f}%</b>", body_style),
                Paragraph(f"Risk Tier: <b><font color='{tier_color}'>{tier_txt}</font></b>", body_style),
                Spacer(1, 8),
                Paragraph("Customer Profile", h2_style),
            ]
            ...
            story.append(Paragraph("Retention Recommendations", h2_style))
            recs_pdf = []
